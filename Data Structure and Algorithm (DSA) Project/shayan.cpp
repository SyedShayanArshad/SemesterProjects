...
private:
    ...
    void deepCopy(const myQueue<T> &other)
    {
        ...
        while (current)
        {
            enqueue(current->data); // Copy each element
...
public:
    ...
    myQueue<T> &operator=(const myQueue<T> &other)
    {
        ...
        {                    // Avoid self-assignment
            clearQueue();    // Clear existing data
...
public:
    ...
    void loadFromFile(ifstream &inFile)
    {
        ...
        getline(inFile, time);
        inFile >> isBook;
        inFile.ignore(); // Ignore the newline character after isBook
    }
    string getDate()
    {
        return date;
...
public:
    ...
    BNode<T> *insert(T val, BNode<T> *Node)
    {
        ...
        // Slot comparison is based on the date and time
    ...
    {
        root = insert(val, root); // Explicitly pass root here
    }
    ...
    void viewPreOrder(BNode<T> *Node)
    {
        if (Node != nullptr)
        {
            Node->data.displaySlots();
            viewPreOrder(Node->left);
            viewPreOrder(Node->right);
        }
    }

    void viewPostOrder(BNode<T> *Node)
    {
        if (Node != nullptr)
        {
            viewPostOrder(Node->left);
            viewPostOrder(Node->right);
            Node->data.displaySlots();
        }
    }
    T Max(BNode<T> *Node)
...
public:
    string status;
    StatusNode *adj; // Linked list of adjacent (transition) nodes
    ...
    {
        StatusNode *newNode = new StatusNode(toStatus->status);
        newNode->adj = adj;
        adj = newNode; // Insert the new transition at the head of the linked list
    }
};
...
public:
    ...
    StatusNode *findStatus(string status)
    {
        ...
        while (temp != nullptr)
        {
            if (temp->status == status)
                return temp;
            temp = temp->adj; // Traverse the list of status nodes
        }
    ...
    bool isValidTransition(string fromStatus, string toStatus)
    {
        ...
        if (from == nullptr || to == nullptr)
            return false;
        StatusNode *adj = from->adj;
        while (adj != nullptr)
        {
            if (adj->status == toStatus)
            {
                return true;
            }
            adj = adj->adj;
    ...
    string getNextStatus(string currentStatus)
    {
        ...
        while (temp != nullptr) // Display all adjacent statuses
        {
            count++;
        ...
        temp = adjacent;
        for (int i = 1; i < choice; i++) // Move to the selected status
        {
            temp = temp->adj;
        }
        return temp->status;
...
public:
    ...
    Appointment() {};
    bool transitionStatus(string newStatus)
    {
        if (statusGraph->isValidTransition(status, newStatus))
        {
            status = newStatus;
        ...
        {
            cout << "Invalid status transition." << endl;
            return false;
        }
    }
    Slot getSlot() const
...
public:
    ...
    void UpdateAppointmentStatus(myQueue<Appointment> &queue)
    {
        ...
        int queueSize = queue.size(); // Get the initial size of the queue

        for (int i = 0; i < queueSize; i++) // Process each appointment once
        {
            Appointment app = queue.frontData();
            queue.dequeue(); // Temporarily remove the front appointment

            // Skip appointments that are already in terminal states
            if (app.getStatus() == "Verified" || app.getStatus() == "Rejected")
            {
                queue.enqueue(app); // Re-enqueue it without processing
            ...
            // Open the document if admin chooses
            ...
            // Re-enqueue the processed appointment back into the queue
            queue.enqueue(app);

            // Ask user if they want to process the next appointment
            char continueChoice;
            if (i < queueSize - 1) // Only ask if there are more appointments to process
            {
    ...
    void trackYourAppointment()
    {
        ...
        while (!tempUrgentQueue.empty())
        {
            ...
            if (app.getUser() == LoginUser)
            {
                cout << "\nUrgent Appointment Found:\n";
                app.displayAppointmentDetails();
                cout << "Current Status: " << app.getStatus() << endl;
                if (app.getStatus() == "In Progress")
                {
                    cout << "Next Possible Stages: ";
        ...
        if (!hasAppointments)
        {
            cout << "You have no appointments to track.\n";
        }
    ...
    void bookNewAppointment()
    {
        ...
        {
            // Handle normal appointment booking
            cout << "Available Normal Slots:\n";
            SlotTree->printInOrder(); // Display normal slots
    ...
    void saveUrgentSlotsToFile()
    {
        ...
        myQueue<Slot> tempQueue = urgentSlotsQueue; // Temporary queue for saving
        while (!tempQueue.empty())
        {
            Slot slot = tempQueue.frontData();
            slot.saveToFile(outFile); // Use Slot's saveToFile method
            tempQueue.dequeue();
        }

        outFile.close();
        cout << "Urgent slots saved successfully to 'UrgentSlots.txt'.\n";
    }
    void loadUrgentSlotsFromFile()
    {
        ...
        while (inFile.peek() != EOF) // Check for end of file
        {
            Slot slot;
            slot.loadFromFile(inFile);      // Use Slot's loadFromFile method
            urgentSlotsQueue.enqueue(slot); // Add the slot to the queue
        }

        inFile.close();
        cout << "Urgent slots loaded successfully from 'UrgentSlots.txt'.\n";
    }
    ...
    void displayAppointments()
    {
        ...
        cout << "\tUrgent Appointments:\n";
        myQueue<Appointment> temp = urgentQueue;
        while (!temp.empty())
        {
            temp.frontData().displayAppointmentDetails();
            temp.dequeue();
        }
        cout << "\tNormal Appointments:\n";
        temp = normalQueue;
        while (!temp.empty())
        {
            temp.frontData().displayAppointmentDetails();
    ...
    void viewUserAppointments(User *loggedInUser)
    {
        system("cls");
        cout << "\t\t\tYour Appointment Report\n";
        bool hasAppointments = false;
        // myQueue<Appointment> tempUrgentQueue = copyQueue(urgentQueue);
        myQueue<Appointment> tempUrgentQueue = urgentQueue;
        myQueue<Appointment> tempNormalQueue = normalQueue;
        cout << "\nUrgent Appointments:\n\n";
        while (!tempUrgentQueue.empty())
        {
            Appointment app = tempUrgentQueue.frontData();
            tempUrgentQueue.dequeue();
            if (app.getUser() == loggedInUser)
            {
                app.displayAppointmentDetails();
                hasAppointments = true;
            }
        }
        // myQueue<Appointment> tempNormalQueue = copyQueue(normalQueue);
        cout << "\nNormal Appointments:\n\n";
        while (!tempNormalQueue.empty())
        {
            Appointment app = tempNormalQueue.frontData();
        ...
        if (!hasAppointments)
        {
            cout << "You have no appointments to display in your report.\n";
        }
    }
    void displayAllAppointmentsReport()
    {
        ...
        while (!tempUrgentQueue.empty())
        {
            ...
            // Only show appointments that are Verified or Rejected
            if (app.getStatus() == "Verified" || app.getStatus() == "Rejected")
            {
                cout << left << setw(5) << "| " << app.getUser()->getUserID()
                     << setw(25) << "| " << app.getUser()->getUserName()
                     << setw(10) << "| " << "Urgent"
                     << setw(15) << "| " << app.getStatus()
                     << setw(40) << "| " << app.getDocumentPath() << " |" << endl;
                cout << "-------------------------------------------------------------------------------------------------------------\n"; // Add line after each row
                hasAppointments = true;
            }
        }

        // Display normal appointments that are Verified or Rejected
        myQueue<Appointment> tempNormalQueue = normalQueue;
        while (!tempNormalQueue.empty())
        {
            Appointment app = tempNormalQueue.frontData();
        ...
        if (!hasAppointments)
        {
            cout << "No appointments found with Verified or Rejected status.\n";
        }
    ...
    void displayUserAppointmentsReport()
    {
        ...
        // Define the headers for the tabular format with adjusted column widths
        cout << left << setw(10) << "| ID "
             << setw(25) << "| User "
             << setw(15) << "| Type "
             << setw(20) << "| Status "
             << setw(50) << "| Document Path " << " |" << endl;
        cout << "-------------------------------------------------------------------------------------------------------------\n";

        bool hasAppointments = false;

        // Display urgent appointments that are Verified or Rejected for the logged-in user
        myQueue<Appointment> tempUrgentQueue = urgentQueue;
        while (!tempUrgentQueue.empty())
        {
            Appointment app = tempUrgentQueue.frontData();
            ...
            // Only show appointments that are Verified or Rejected and belong to the logged-in user
            if ((app.getUser() == LoginUser) && (app.getStatus() == "Verified" || app.getStatus() == "Rejected"))
            {
                cout << left << setw(10) << "| " << app.getUser()->getUserID()
                     << setw(25) << "| " << app.getUser()->getUserName()
                     << setw(15) << "| " << "Urgent"
                     << setw(20) << "| " << app.getStatus()
                     << setw(50) << "| " << app.getDocumentPath() << " |" << endl;
                cout << "-------------------------------------------------------------------------------------------------------------\n"; // Add line after each row
                hasAppointments = true;
            }
        }

        // Display normal appointments that are Verified or Rejected for the logged-in user
        myQueue<Appointment> tempNormalQueue = normalQueue;
        while (!tempNormalQueue.empty())
        {
            Appointment app = tempNormalQueue.frontData();
        ...
        if (!hasAppointments)
        {
            cout << "You have no appointments with Verified or Rejected status.\n";
        }
...
public:
    ...
    {
        system("cls");
        cout << "All Available Appointment Slots: \n";
        cout << "Press any key to continue...";
        _getch();
    }
};
...
void UserMainMenu()
{
    ...
    do
    {
        ...
        case 3:
            ...
            cout << "Cancel Appointment functionality coming soon.\n";
            _getch();
            break;
