...
public:
    ...
    void dequeue()
    {
        if (empty())
        {
            throw runtime_error("Queue underflow: cannot pop from an empty queue.");
        }
    ...
    T backData()
    {
        if (empty())
        {
            throw runtime_error("Queue is empty: cannot access rear element.");
        }
        return rear->data;
    }
};
...
public:
    string status;
    StatusNode *nextStatus; // Pointer to the next node in the list (for transitions)
    StatusNode *adj;        // Linked list of adjacent (transition) nodes
    StatusNode *next;       // Pointer to the next StatusNode in the graph (used for traversal)
...
public:
    ...
    string getNextStatus(string currentStatus)
    {
        ...
        // Return the first adjacent status
        if (current->adj != nullptr)
        {
            return current->adj->status;
        }
...
StatusGraph *setupStatusGraph()
{
    ...
    statusGraph->addTransition("Verified", "Rejected");
    return statusGraph;
}
...
public:
    ...
    void UpdateAppointmentStatus()
    {
        ...
        while (!tempUrgentQueue.empty())
        {
            ...
            cout << "Possible Next Statuses:\n";
            myStatusGraph->displayNextStatuses(app.getStatus());
            cout << "Do you want to move the status to next Step? (y/n): ";
            char moveChoice;
            cin >> moveChoice;
            ...
            {
                cout << "Skipping this appointment.\n";
            }

            cout << "Press any key to continue to the next appointment...\n";
            getch();
        }
        ...
        while (!tempNormalQueue.empty())
        {
            ...
            // Show current status and possible next statuses
            cout << "\nCurrent Status: " << app.getStatus() << endl;
            cout << "Possible Next Statuses:\n";
            myStatusGraph->displayNextStatuses(app.getStatus());

            // Ask if admin wants to move to the next status
            cout << "Do you want to move to the next status? (y/n): ";
            char moveChoice;
            cin >> moveChoice;

            if (moveChoice == 'y' || moveChoice == 'Y')
            {
                // Get the next status from the graph and update the appointment
                string nextStatus = myStatusGraph->getNextStatus(app.getStatus());
                if (!nextStatus.empty())
                {
                    app.setStatus(nextStatus);
        ...
        cout << "All appointments have been processed.\n";
...
void UserMainMenu()
{
    ...
    do
    {
        ...
        case 7:
            appointmentQueue.viewUserAppointments(LoginUser);
            cout << "Press any key to go back.\n";
            getch();
            break;
